...
`,i+=`
`,r===`coaching`?i+=x(e):r===`manipulation`?i+=S(e):r===`channels`?i+=C(e):r===`paradigm`?i+=w(e):r===`diagnosis`?i+=T(e):r===`needs-dependency`?i+=D(e):r===`sovereignty`||r===`sovereignty-analysis`?i+=k(e):r===`sovereignty-paradigm`&&(i+=O(e)),i+=`
...
`,i+=`
`,i}function x(e){let t=`=== COACHING PROFILE DATA ===
...
`,e.secondaryConsiderations.forEach(e=>{t+=`"${e.name}",${e.probability}%,${e.severity||`Not specified`}\n`})),t}function E(e){return e==null||e===``?`Not answered`:typeof e==`object`?e.mapsTo&&e.mapsTo.need?e.mapsTo.need:e.text?e.text:Array.isArray(e)?e.map(e=>E(e)).join(`; `):JSON.stringify(e):String(e)}function D(e){let t=`=== NEEDS DEPENDENCY LOOP DETERMINATOR DATA (4-Phase Architecture) ===
`;if(e.questionSequence&&e.questionSequence.length>0&&(t+=`
=== ALL QUESTIONS AND ANSWERS ===
`,t+=`Question ID,Question Text,Answer,Phase,Loop
`,e.questionSequence.forEach(n=>{let r=E(n.answer===void 0?e.allAnswers&&e.allAnswers[n.id]:n.answer),i=(n.question||n.questionText||``).replace(/"/g,`""`);t+=`"${n.id}","${i}","${String(r).replace(/"/g,`""`)}",${n.phase||``},"${(n.loop||``).replace(/"/g,`""`)}"\n`})),e.primaryLoop){t+=`
...
`;function M(e){return e==null?``:String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}function N(e){let t=new TextEncoder().encode(e),n=``;for(let e=0;e<t.length;e++)n+=String.fromCharCode(t[e]);let r=btoa(n),i=[];for(let e=0;e<r.length;e+=76)i.push(r.slice(e,e+76));return i.join(`\r
`)}function P(e={}){let{title:t=`Assessment report`,filenameBase:n=`report`,rootSelector:r,rootSelectors:i}=e,a=i?Array.isArray(i)?i:[i]:r?[r]:[];if(!a.length||typeof document>`u`)return!1;let o=[];for(let e of a){let t=document.querySelector(e);if(!t)continue;let n=t.cloneNode(!0);n.querySelectorAll(`button, .export-section, [data-no-export], script, style[data-live-only]`).forEach(e=>{e.remove()}),o.push(`<div class="report-export-section">${n.innerHTML}</div>`)}if(!o.length)return!1;let s=M(t),c=`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<body>
<h1>${s}</h1>
<p class="report-export-meta">Exported ${M(new Date().toISOString())}</p>
${o.join(`
`)}
</body>
</html>`,l=String(n).replace(/[^a-z0-9-_]+/gi,`_`).replace(/^_|_$/g,``)||`report`,u=`BMReport_${Date.now()}_${Math.random().toString(36).slice(2,10)}`,d=`file:///C:/${l}.html`,f=String(t).replace(/[\r\n]+/g,` `).slice(0,200),p=N(c);return F([`From: <Saved by Belief Mastery>`,`Snapshot-Content-Location: ${d}`,`Subject: ${f}`,`Date: ${new Date().toUTCString()}`,`MIME-Version: 1.0`,`Content-Type: multipart/related; type="text/html"; boundary="${u}"`,``,`--${u}`,`Content-Type: text/html; charset=utf-8`,`Content-Transfer-Encoding: base64`,`Content-Location: ${d}`,``,p,``,`--${u}--`,``].join(`\r
`),`${l}.mhtml`,`multipart/related`),!0}function F(e,t,n){let r=new Blob([e],{type:n}),i=URL.createObjectURL(r),a=document.createElement(`a`);a.href=i,a.download=t,a.click(),URL.revokeObjectURL(i)}function I(e,t={}){e.externalUI=!!t.externalUI,e._spaNotify=typeof t.onNotify==`function`?t.onNotify:null,e._spaPhase=e._spaPhase||`idle`,e._externalQuestionSnapshot=e._externalQuestionSnapshot??null}function L(e,t,n){try{e._spaNotify?.(t,n)}catch(e){console.warn(`Engine SPA notify:`,e)}}function R(e,t){e._spaPhase=t,L(e,`phase`,{phase:t})}export{P as a,A as c,u as d,c as f,i as h,F as i,d as l,a as m,L as n,y as o,o as p,R as r,b as s,I as t,l as u};