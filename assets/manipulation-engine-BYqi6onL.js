import{_ as e,c as t,d as n,f as r,g as i,h as a,l as o,m as s,p as c,t as l,u,v as d,y as f}from"./spa-engine-external-BwN7bktj.js";import{t as p}from"./debug-reporter-B-MS2Y6w.js";import{a as m,d as h,g,h as _,i as v,m as y,n as b,o as x,r as S,t as C,u as w,v as T}from"./spa-questionnaire-host-CPgiO3Fd.js";var E,D,O,k,A,j,M,N=class{constructor(e={}){l(this,e),this.currentPhase=1,this.currentQuestionIndex=0,this.answers={},this.questionSequence=[],this.vectorScores={},this.prioritizedVectors=[],this.assessedVectors=[],this.reportComplete=!1,this.analysisData={timestamp:new Date().toISOString(),phase1Results:{},phase2Results:{},phase3Results:{},vectorScores:{},identifiedVectors:[],primaryVector:null,supportingVectors:[],tactics:[],structuralModifier:null,allAnswers:{},questionSequence:[]},this.debugReporter=p(`ManipulationEngine`),f(this.debugReporter),this.debugReporter.markInitialized(),this.dataStore=new a(`manipulation-assessment`,`1.0.0`),this.ui=b(this,{idle:{show:[`#introSection`,`#actionButtonsSection`],hide:[`#questionnaireSection`,`#resultsSection`]},assessment:{show:[`#questionnaireSection`],hide:[`#introSection`,`#actionButtonsSection`,`#resultsSection`]},results:{show:[`#resultsSection`],hide:[`#introSection`,`#actionButtonsSection`,`#questionnaireSection`]}}),this.ready=this.init()}init(){return this.externalUI||this.attachEventListeners(),Promise.resolve(this.loadStoredData()).then(()=>{if(!this.externalUI&&this.shouldAutoGenerateSample())return this.generateSampleReport();v(this)}).catch(e=>{this.debugReporter.logError(e,`init`)})}async loadManipulationData(){if(!(E&&j))try{E=(await d(`./manipulation-data/manipulation-vectors.js`,`Manipulation Vectors`)).MANIPULATION_VECTORS,D=(await d(`./manipulation-data/manipulation-tactics.js`,`Manipulation Tactics`)).MANIPULATION_TACTICS,O=(await d(`./manipulation-data/symptom-questions.js`,`Symptom Questions`)).SYMPTOM_QUESTIONS,k=(await d(`./manipulation-data/effect-questions.js`,`Effect Questions`)).EFFECT_QUESTIONS,A=(await d(`./manipulation-data/consequence-questions.js`,`Consequence Questions`)).CONSEQUENCE_QUESTIONS,(await d(`./manipulation-data/vector-mapping.js`,`Vector Mapping`)).VECTOR_MAPPING;let e=await d(`./manipulation-data/manipulation-questions-v2.js`,`Manipulation Questions`);j=e.PHASE_1_VECTOR_SCREENING,M=e.generatePhase3VectorQuestions,this.debugReporter.recordSection(`Phase 1`,j?.length||0)}catch(e){throw this.debugReporter.logError(e,`loadManipulationData`),i.showUserError(`Failed to load assessment data. Please refresh the page.`),e}}attachEventListeners(){let e=document.getElementById(`startAssessment`);e&&e.addEventListener(`click`,()=>this.startAssessment());let n=document.getElementById(`nextQuestion`);n&&n.addEventListener(`click`,()=>this.nextQuestion());let r=document.getElementById(`prevQuestion`);r&&r.addEventListener(`click`,()=>this.prevQuestion());let i=document.getElementById(`exportReportHtml`);i&&i.addEventListener(`click`,()=>this.exportReportHtml());let a=document.getElementById(`exportExecutiveBrief`);a&&a.addEventListener(`click`,()=>this.exportExecutiveBrief());let o=document.getElementById(`newAssessment`);o&&o.addEventListener(`click`,()=>this.resetAssessment());let s=document.getElementById(`generateSampleReport`);s&&s.addEventListener(`click`,()=>this.generateSampleReport());let c=document.getElementById(`abandonAssessment`);c&&c.addEventListener(`click`,async()=>{await t(`Are you sure you want to abandon this assessment? All progress will be lost.`)&&this.resetAssessment()}),document.addEventListener(`keydown`,e=>{e.key===`Enter`&&e.target.matches(`button, input[type="radio"], input[type="checkbox"]`)||(e.key===`ArrowRight`||e.key===`Enter`&&e.ctrlKey?(e.preventDefault(),this.nextQuestion()):(e.key===`ArrowLeft`||e.key===`Backspace`&&e.ctrlKey)&&(e.preventDefault(),this.prevQuestion()))})}shouldAutoGenerateSample(){let e=new URLSearchParams(window.location.search);if(!e.has(`sample`))return!1;let t=e.get(`sample`);return t===null||t===``||t===`1`||t===`true`}getEmptyAnalysisData(){return{timestamp:new Date().toISOString(),phase1Results:{},phase2Results:{},phase3Results:{},prioritizedVectors:[],assessedVectors:[],vectorScores:{},primaryVector:null,secondaryVectors:[],allAnswers:{},questionSequence:[]}}pickRandomIndices(e,t){let n=Array.from({length:e},(e,t)=>t);for(let e=n.length-1;e>0;--e){let t=Math.floor(Math.random()*(e+1));[n[e],n[t]]=[n[t],n[e]]}return n.slice(0,t)}answerQuestionForSample(e){if(!e||!Array.isArray(e.options)||e.options.length===0)return;if(e.type===`multiselect`){let t=e.maxSelections||3,n=Math.min(e.options.length,Math.max(1,Math.ceil(Math.random()*t))),r=this.pickRandomIndices(e.options.length,n).map(t=>e.options[t]);this.answers[e.id]=r,this.currentPhase===3&&this.processPhase3Answer(e);return}let t=e.options[Math.floor(Math.random()*e.options.length)];this.answers[e.id]=t,this.currentPhase===3&&this.processPhase3Answer(e)}async generateSampleReport(){try{if(await this.loadManipulationData(),this.currentPhase=1,this.currentQuestionIndex=0,this.answers={},this.questionSequence=[],this.prioritizedVectors=[],this.assessedVectors=[],this.analysisData=this.getEmptyAnalysisData(),await this.buildPhase1Sequence(),this.questionSequence.forEach(e=>this.answerQuestionForSample(e)),await this.analyzePhase1Results(),this.questionSequence.forEach(e=>this.answerQuestionForSample(e)),await this.processPhase2Results(),this.currentPhase===3&&this.questionSequence.length>0){for(let e=0;e<this.questionSequence.length;e+=1){let t=this.questionSequence[e];this.answerQuestionForSample(t)}this.processPhase3Results()}this.completeAssessment()}catch(e){this.debugReporter.logError(e,`generateSampleReport`),i.showUserError(`Failed to generate sample report. Please try again.`)}}startAssessment(){this.ui.transition(`assessment`),this.buildPhase1Sequence()}async buildPhase1Sequence(){await this.loadManipulationData(),this.questionSequence=[],this.currentPhase=1,this.currentQuestionIndex=0,this.questionSequence.push(...j),this.debugReporter.recordQuestionCount(this.questionSequence.length);let e=document.getElementById(`questionnaireSection`);e&&!e.classList.contains(`active`)&&e.classList.add(`active`),this.renderCurrentQuestion()}async analyzePhase1Results(){await this.loadManipulationData();try{this.vectorScores={},Object.keys(E).forEach(e=>{let t=E[e],n=0,r=0;j.forEach(t=>{let i=this.answers[t.id];if(i&&i.mapsTo&&i.mapsTo.vector===e){let e=i.mapsTo.state,t=i.mapsTo.weight||1;n+=(e===`high`?3:+(e===`medium`))*t,r+=t}});let i=r>0?n/r:0,a=i>=2?`high`:i>=.5?`medium`:`low`;this.vectorScores[e]={state:a,score:i,vector:t}}),this.analysisData.phase1Results=this.vectorScores,await this.buildPhase2Sequence()}catch(e){this.debugReporter.logError(e,`analyzePhase1Results`),i.showUserError(`Failed to analyze Phase 1 results. Please try again.`)}}async buildPhase2Sequence(){await this.loadManipulationData();try{this.questionSequence=[],this.currentPhase=2,this.currentQuestionIndex=0;let e=Object.keys(this.vectorScores).filter(e=>this.vectorScores[e].state===`high`||this.vectorScores[e].state===`medium`).map(e=>({id:e,vector:E[e],score:this.vectorScores[e].score,state:this.vectorScores[e].state})).sort((e,t)=>t.score-e.score).slice(0,6);this.questionSequence.push(_({id:`p2_prioritization`,question:`Based on your initial screening, how much do you want to explore each manipulation vector?`,type:`multiselect`,maxSelections:3,options:e.map(e=>({text:`${e.vector.name}: ${e.vector.description}`,mapsTo:{vector:e.id,priority:e.state===`high`?`high`:`medium`},vector:e.id})),phase:2,likelyVectors:e},{convertMultiselect:!0})),this.debugReporter.recordQuestionCount(this.questionSequence.length),this.renderCurrentQuestion()}catch(e){this.debugReporter.logError(e,`buildPhase2Sequence`),i.showUserError(`Failed to load Phase 2. Please refresh the page.`)}}async processPhase2Results(){try{let e=this.answers.p2_prioritization,t=this.questionSequence.find(e=>e.id===`p2_prioritization`);e?.weights&&t?.allocationMembers?(this.prioritizedVectors=T(e,t.allocationMembers,e=>e.mapsTo?.vector,12,3),this.analysisData.prioritizedVectors=this.prioritizedVectors):Array.isArray(e)&&(this.prioritizedVectors=e.map(e=>e.mapsTo.vector),this.analysisData.prioritizedVectors=this.prioritizedVectors),await this.buildPhase3Sequence()}catch(e){this.debugReporter.logError(e,`processPhase2Results`),i.showUserError(`Failed to process Phase 2 results.`)}}async buildPhase3Sequence(){await this.loadManipulationData();try{this.questionSequence=[],this.currentPhase=3,this.currentQuestionIndex=0;let e={symptoms:[],effects:[],consequences:[]};Object.keys(O).forEach(t=>{let n=O[t];Array.isArray(n)&&e.symptoms.push(...n)}),Object.keys(k).forEach(t=>{let n=k[t];Array.isArray(n)&&e.effects.push(...n)}),Object.keys(A).forEach(t=>{let n=A[t];Array.isArray(n)&&e.consequences.push(...n)}),this.prioritizedVectors.forEach(t=>{let n=E[t];n&&M(t,n,e).forEach(e=>{this.questionSequence.push({...e,phase:3,vector:t})})}),this.renderCurrentQuestion()}catch(e){this.debugReporter.logError(e,`buildPhase3Sequence`),i.showUserError(`Failed to build Phase 3 sequence. Please try again.`)}}processPhase3Answer(e){if(e.type===`binary_unsure`){let t=this.answers[e.id];if(t&&t.text){let n=e.conditional&&e.conditional[t.text];if(n){let t=this.questionSequence.findIndex(t=>t.id===e.id);g(n).forEach((n,r)=>{this.questionSequence.splice(t+1+r,0,{...n,phase:3,vector:e.vector})})}}}else if(e.type===`multiselect`&&e.conditional&&e.conditional.selected){let t=this.answers[e.id];if(t&&Array.isArray(t)&&t.length>0){let n=t.map(e=>e.mapsTo?.symptomId||e.mapsTo?.effectId||e.mapsTo?.consequenceId).filter(Boolean),r=e.conditional.selected.filter(e=>{if(e.conditional!==!0)return!1;let t=e.originalQuestion?.id;return t&&n.includes(t)});if(r.length>0){let t=this.questionSequence.findIndex(t=>t.id===e.id);r.forEach((n,r)=>{this.questionSequence.findIndex(e=>e.id===n.id)===-1&&this.questionSequence.splice(t+1+r,0,{...n,phase:3,vector:e.vector})})}}}}renderCurrentQuestion(){let t=performance.now();if(this.currentQuestionIndex>=this.questionSequence.length){this.completePhase();return}let n=this.questionSequence[this.currentQuestionIndex];if(S(this,n,{totalQuestions:this.questionSequence.length,questionIndex:this.currentQuestionIndex,phase:this.currentPhase,stageLabel:this.getPhaseLabel(this.currentPhase)})){this.updateProgress(),this.updateNavigationButtons();return}let r=this.externalUI?this._externalQuestionMount:document.getElementById(`questionContainer`);if(!r){this.externalUI||i.showUserError(`Question container not found. Please refresh the page.`);return}try{let i=``;n.type===`allocation`?i=h(n,this.answers[n.id],{phase:this.currentPhase,questionIndex:this.currentQuestionIndex,questionTotal:this.questionSequence.length,stageLabel:this.getPhaseLabel(this.currentPhase)}):n.type===`three_point`?i=this.renderThreePointQuestion(n):n.type===`binary_unsure`?i=this.renderBinaryUnsureQuestion(n):n.type===`frequency`?i=this.renderFrequencyQuestion(n):n.type===`multiselect`&&(i=this.renderMultiselectQuestion(n)),e.safeInnerHTML(r,i),this.attachQuestionListeners(n),n.type===`allocation`&&w(r,this,n),this.updateProgress(),this.updateNavigationButtons();let a=performance.now()-t;this.debugReporter.recordRender(`question`,a),setTimeout(()=>{r.scrollIntoView({behavior:`smooth`,block:`start`})},100);let o=r.querySelector(`input, button, select, textarea`);o&&s.focusElement(o)}catch(e){this.debugReporter.logError(e,`renderCurrentQuestion`),i.showUserError(`Failed to render question. Please refresh the page.`)}}renderThreePointQuestion(t){let n=this.answers[t.id];return`
    ...
    `}renderPhase2Prioritization(t,n,r){let i=Object.keys(this.vectorScores).filter(e=>this.vectorScores[e].state===`high`).map(e=>E[e].name),a=Object.keys(this.vectorScores).filter(e=>this.vectorScores[e].state===`medium`).map(e=>E[e].name),o=`
      <div class="question-block">
        <div class="question-header">
          <span class="question-number">Phase ${this.currentPhase} - Question ${this.currentQuestionIndex+1} of ${this.questionSequence.length}</span>
          <span class="question-stage">${this.getPhaseLabel(this.currentPhase)}</span>
        ...
        <div class="multiselect-options">
    `;return t.options.forEach((r,i)=>{let a=n.some(e=>e.text===r.text),s=E[r.mapsTo.vector];o+=`
        <label class="multiselect-option ${a?`selected`:``}">
          <input 
            type="checkbox" 
            name="question_${t.id}" 
