...
4. **Questionnaire complete** → `calculateResults()` called
5. **Good Life calculation** → `calculateGoodLifeScores()` (if selected)
6. **God calculation** → `calculateGodScores()` (if selected)
7. **Paradigm identification** → `identifyParadigms()` finds primaries
8. **Results display** → `renderResults()` shows results
