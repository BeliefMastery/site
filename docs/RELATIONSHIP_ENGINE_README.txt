...
3. **Stage 1 complete** â†’ `analyzeStage1Results()` identifies weakest links
4. **If weak areas found** â†’ `buildStage2Sequence()` creates Stage 2 questions
5. **User answers Stage 2** â†’ Answers stored
6. **Stage 2 complete** â†’ `analyzeStage2Results()` analyzes domains
7. **If critical areas** â†’ `buildStage3Sequence()` creates Stage 3 questions
8. **User answers Stage 3** â†’ Answers stored
9. **Stage 3 complete** â†’ `analyzeStage3Results()` analyzes scenarios
10. **Finalize results** â†’ `calculateResults()` and `finalizeResults()` compile data
