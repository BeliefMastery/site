...
1. **Engine initializes** → `buildQuestionSequence()` creates sequence
2. **User answers questions** → Answers stored in `answers`
3. **Questionnaire complete** → `calculateResults()` called
4. **Dimension calculation** → Groups answers and calculates dimension scores
5. **Overall calculation** → Calculates overall temperament
6. **Category determination** → Determines temperament category
7. **Variation analysis** → `analyzeVariation()` identifies patterns
8. **Results display** → `renderResults()` shows analysis
9. **Export** → User exports analysis
