...
3. **Questionnaire complete** → `completeAssessment()` called
4. **Vector calculation** → `calculateVectorScores()` processes answers
5. **Vector identification** → `identifyVectors()` finds exceeding thresholds
6. **Tactic identification** → `identifyTactics()` finds relevant tactics
7. **Results display** → `renderResults()` shows analysis
8. **Export** → User exports analysis
