...
1. **User selects categories** → `selectedCategories` populated
2. **User starts assessment** → `buildQuestionSequence()` creates question sequence
3. **User answers questions** → Answers stored in `answers` object
4. **Questionnaire complete** → `completeAssessment()` called
5. **Comorbidity check** → `detectComorbidity()` analyzes patterns
6. **Refinement (optional)** → Additional questions if comorbidity detected
7. **Results calculation** → `calculateResults()` processes all answers
8. **Results display** → `renderResults()` shows final results
9. **Export** → User can export results in multiple formats
