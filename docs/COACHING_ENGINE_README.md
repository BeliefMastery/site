...
1. **User selects sections** → `selectedSections` populated
2. **User starts assessment** → `buildQuestionSequence()` creates sequence
3. **User answers questions** → Answers stored in `answers`
4. **Questionnaire complete** → `completeAssessment()` called
5. **Profile calculation** → `calculateProfile()` processes answers
6. **Priority identification** → `calculatePriorities()` finds top items
7. **Coaching profile** → `generateCoachingProfile()` creates AI config
8. **Results display** → `renderResults()` shows profile
9. **Export** → User exports profile for AI agent
