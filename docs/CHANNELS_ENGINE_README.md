...
**Stage Completion:**
- If nodes with issues found → Proceed to Stage 2
- If no nodes with issues → Show results (system healthy)
...
**Stage Completion:**
- If blocked channels found → Proceed to Stage 3
- If no blocked channels → Show results
...
3. **Stage 1 complete** → `analyzeStage1Results()` identifies nodes with issues
4. **If nodes identified** → `buildStage2Sequence()` creates Stage 2 questions
5. **User answers Stage 2** → Answers stored
6. **Stage 2 complete** → `analyzeStage2Results()` identifies blocked channels
7. **If channels blocked** → `buildStage3Sequence()` creates Stage 3 questions
8. **User answers Stage 3** → Answers stored
9. **Stage 3 complete** → `analyzeStage3Results()` analyzes management
10. **Finalize results** → `finalizeResults()` compiles all data
...
- Channels are directional (from → to)
